...
export default function ProfilePage() {
  ...
  const isFetching = Boolean(user) && (isLoading || fetchedUid !== user?.uid)
          ...
          <div className='flex flex-col sm:flex-row items-stretch sm:items-center gap-3'>
            <div className='flex items-center gap-4 px-4 py-3 bg-slate-900/80 border border-white/10 rounded-xl'>
              ...
              <div>
                ...
                <p
                  className={`text-sm ${user.displayName ? "text-slate-400" : "font-bold text-white"}`}
                >
                  {user.email}
                </p>
              </div>
        ...
        <div className='grid grid-cols-2 md:grid-cols-3 gap-4'>
          {[
            ...
            },
            {
              icon: Package,
              label: "W realizacji",
              value: transactions.filter((t) => t.status !== "delivered")
                .length,
              color: "text-blue-400",
                ...
                >
                  ...
                  <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-3'>
                    ...
                    <div className='flex items-center gap-3'>
                      <span
                        className={`px-3 py-1 rounded-full border text-xs font-bold uppercase tracking-wider ${transactionStatusColors[txn.status]}`}
                      >
                  ...
                  <div className='flex flex-wrap gap-3'>
                    {txn.items.map((item, j) => (
                      <div
                        key={j}
                        className='flex items-center gap-2 bg-slate-900 border border-white/5 rounded-lg p-2'
                      >
                        {item.image && (
                  ...
                  <div className='flex items-center gap-4 text-xs text-slate-500 border-t border-white/5 pt-3'>
                    {txn.deliveryMethod === "courier" ? (
                      <span className='flex items-center gap-1'>
                        <Truck className='w-3.5 h-3.5 text-blue-400' /> Kurier
                        chłodniczy
                      </span>
                    ) : (
                      <span className='flex items-center gap-1'>
